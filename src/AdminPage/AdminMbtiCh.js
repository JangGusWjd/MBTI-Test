...
const AdminMbtiCh = ({ inputValue }) => {
  ...
  const newMbtiInfo = inputValue;
  const mbti = selectEorI + selectSorN + selectTorF + selectJorP;
  const navigate = useNavigate();

  const addMbti = async () => {
    try {
      await axios.post("http://49.247.33.67:8080/mbti", {
        mbti: mbti,
        info: newMbtiInfo,
      });
  ...
  return (
    <div className="mbti-container">
      ...
      <ResultBtn onClick={addMbti}>MBTI 추가</ResultBtn>
    </div>
  );
};

export default AdminMbtiCh;
